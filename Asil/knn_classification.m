...
for dIdx = 1:length(distanceMetrics)
    ...
    plot(K_neighbors, accVals, '-o', 'Color', colors(dIdx,:), ...
        'LineWidth', 2, 'MarkerSize', 6, 'DisplayName', distanceMetrics{dIdx});
end
xlabel('K (Número de Vecinos)');
ylabel('Precisión de Clasificación (%)');
title(sprintf('k-NN: Precisión vs K (PCA=%d, Ponderación=%s)', overallBestConfig.PCA_dim, overallBestConfig.weighting));
legend('Location', 'best');
grid on;
xlim([min(K_neighbors)-1, max(K_neighbors)+1]);
saveas(gcf, fullfile(figuresDir, 'knn_precision_vs_k.png'));
...
for dIdx = 1:length(distanceMetrics)
    ...
    plot(PCA_dims, accVals, '-s', 'Color', colors(dIdx,:), ...
        'LineWidth', 2, 'MarkerSize', 6, 'DisplayName', distanceMetrics{dIdx});
end
xlabel('Dimensiones PCA');
ylabel('Precisión de Clasificación (%)');
title(sprintf('k-NN: Precisión vs Dimensiones PCA (K=%d, Ponderación=%s)', overallBestConfig.K, overallBestConfig.weighting));
legend('Location', 'best');
grid on;
saveas(gcf, fullfile(figuresDir, 'knn_precision_vs_pca.png'));
fprintf('  Guardado: precision_vs_pca.png\n');

figure('Position', [100, 100, 800, 600]);
hold on;
plot(PCA_dims, results.pcaVariance, '-o', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('Dimensiones PCA');
ylabel('Varianza Explicada (%)');
title('PCA: Varianza Explicada vs Número de Componentes');
grid on;
yline(95, '--r', '95%');
yline(99, '--g', '99%');
saveas(gcf, fullfile(figuresDir, 'pca_varianza_explicada.png'));
...
colorbar;
xlabel('Dimensiones PCA');
ylabel('K (Número de Vecinos)');
title(sprintf('Mapa de Calor Precisión k-NN (Distancia: %s, Ponderación: %s)', distanceMetrics{bestDistIdx}, overallBestConfig.weighting));
xticks(1:length(PCA_dims));
xticklabels(string(PCA_dims));
xtickangle(45);
yticks(1:length(K_neighbors));
yticklabels(string(K_neighbors));
for kIdx = 1:length(K_neighbors)
    for pIdx = 1:length(PCA_dims)
        if ~isnan(accMatrix(kIdx, pIdx))
            text(pIdx, kIdx, sprintf('%.1f', accMatrix(kIdx, pIdx)), ...
                'HorizontalAlignment', 'center', 'Color', 'k', 'FontSize', 8);
        end
    end
end
saveas(gcf, fullfile(figuresDir, 'knn_precision_mapa_calor.png'));
fprintf('  Guardado: precision_mapa_calor.png\n');

if ~isempty(overallBestConfig.preds)
    figure('Position', [100, 100, 800, 700]);
    confusionchart(Y_test, overallBestConfig.preds, ...
        'Title', sprintf('Matriz de Confusión (Mejor: K=%d, PCA=%d, %s)', ...
        overallBestConfig.K, overallBestConfig.PCA_dim, overallBestConfig.distance), ...
        'ColumnSummary', 'column-normalized', ...
        'RowSummary', 'row-normalized');
    saveas(gcf, fullfile(figuresDir, 'knn_matriz_confusion.png'));
...
for dIdx = 1:length(distanceMetrics)
    ...
    plot(K_neighbors, timeVals, '-^', 'Color', colors(dIdx,:), ...
        'LineWidth', 2, 'MarkerSize', 6, 'DisplayName', distanceMetrics{dIdx});
end
xlabel('K (Número de Vecinos)');
ylabel('Tiempo de Cómputo (segundos)');
title(sprintf('k-NN: Tiempo de Entrenamiento vs K (PCA=%d, Ponderación=%s)', overallBestConfig.PCA_dim, overallBestConfig.weighting));
legend('Location', 'best');
grid on;
saveas(gcf, fullfile(figuresDir, 'knn_tiempo_vs_k.png'));
...
bar(1:length(weightingMethods), weightingAcc);
set(gca, 'XTick', 1:length(weightingMethods), 'XTickLabel', weightingMethods);
ylabel('Precisión de Clasificación (%)');
title(sprintf('Comparación de Ponderación de Distancia (K=%d, PCA=%d, Distancia=%s)', bestK, bestPCA, bestDist));
grid on;
for i = 1:length(weightingMethods)
    if ~isnan(weightingAcc(i))
        text(i, weightingAcc(i), sprintf('%.2f', weightingAcc(i)), ...
            'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    end
end
saveas(gcf, fullfile(figuresDir, 'knn_comparacion_ponderacion.png'));
